...
def handler(event, context):
    ...
    if not image_id:
        return {"statusCode": 400, "headers": HEADERS, "body": json.dumps({"error": "Missing image id"})}

    dynamodb = boto3.resource("dynamodb")
    table = dynamodb.Table(os.environ["IMAGES_TABLE"])

    item = table.get_item(Key={"id": image_id}).get("Item")
    if not item:
        return {"statusCode": 404, "headers": HEADERS, "body": json.dumps({"error": "Image not found"})}

    table.delete_item(Key={"id": image_id})

    return {"statusCode": 200, "headers": HEADERS, "body": json.dumps({"message": "Image deleted"})}
