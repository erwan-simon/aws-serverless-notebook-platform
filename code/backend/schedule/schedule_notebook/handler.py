...
def handler(event, context):
    ...
    schedule_timezone = body.get("timezone") or "UTC"
    ...
    if not cron_expression or not iam_role_arn or not ecr_image_uri:
        return {
            ...
            "headers": HEADERS,
            "body": json.dumps(
                {"error": "Missing cron_expression, iam_role_arn, or ecr_image_uri"}
            ),
        }
    ...
    logger.info(
        "Scheduling notebook: id=%s, cron=%s, image=%s",
        notebook_id,
        cron_expression,
        ecr_image_uri,
    )
    ...
    return {
        ...
        "headers": HEADERS,
        "body": json.dumps(
            {"message": "Schedule created", "schedule_name": schedule_name}
        ),
    }
